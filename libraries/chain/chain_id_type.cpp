#include <eosio/chain/chain_id_type.hpp>
#include <eosio/chain/exceptions.hpp>

using eosio::chain::hash256;

namespace eosio { namespace chain {

   void chain_id_type::reflector_init()const {
      EOS_ASSERT( *reinterpret_cast<const hash256*>(this) != hash256(), chain_id_type_exception, "chain_id_type cannot be zero" );
   }

} }  // namespace eosio::chain

namespace fc {

   void to_variant(const eosio::chain::chain_id_type& cid, fc::variant& v) {
      to_variant( static_cast<const hash256&>(cid), v);
   }

   void from_variant(const fc::variant& v, eosio::chain::chain_id_type& cid) {
      from_variant( v, static_cast<hash256&>(cid) );
   }

} // fc
